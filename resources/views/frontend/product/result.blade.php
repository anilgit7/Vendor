<div class="py-[3rem]">
    <div class="px-[1rem] 2xs:px-[1.5rem] sm:px-[3rem] lg:px-[5.9rem]">
        <div class="flex flex-col space-y-[5rem] relative">
            <div class="bg-[]">
                <h1 class="font-bold drop-shadow-[0px_0px_.5px_#000000] text-[1.2rem]">Result : 
                    @if($searches)
                        {{$searches->count()}} 
                    @else 
                        0 
                    @endif 
                    items found for "{{$search_name}}"</h1>
            </div>
            @if($searches)
                            ...
                            <div class="flex flex-col space-y-[1rem] group">
                                ...
                                <div class="capitalize flex flex-col items-start font-semibold space-y-[.5rem]">
                                    ...
                                    <div class=" flex justify-between  w-full items-center">
                                        ...
                                        <form class="ajax-cart-form" id="addToCart-{{ $search->id }}" action="{{route("cart.add")}}" method="post">
                                            @csrf
                                            <input type="hidden" name="product_id" value="{{$search->id}}" id="productId">
