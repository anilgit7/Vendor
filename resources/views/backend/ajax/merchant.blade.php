...
$(document).ready(function(){
    ...
    function loadVerfied(){
        $.ajax({
            url : "{{ route('admin.merchant.verified.list') }}",
            success : function(result){
                if(result.users.length>0){
                    $('#verified_merchant').html('');
                    $('#verified_merchant').html(
                        `<tr class="border-b">
                            <th class="text-start pr-[2rem] pb-[.5rem]">S.N</th>
                            <th class="text-start pr-[2rem] pb-[.5rem]">Merchant Name</th>
    ...
    $('#verified_merchant').on('click','.delete',function(){
        var user_id = $(this).attr('data-merchant-id');
        var url = "{{ route('admin.merchant.verified.delete','user_id') }}";
        url = url.replace('user_id',user_id);
        $.ajax({
            url : url,
            success : function(result){
