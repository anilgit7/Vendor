            ...
            <table class="w-full gap-[2rem] space-x-[2rem] rounded-md" id="product_table" style="text-wrap: nowrap;">
                ...
                <tbody id="product_data">
                    ...
                    <tr class="border-b">
                        ...
                        <td class="py-[.5rem] pr-[2rem] capitalize"><a href="{{route('merchant.order.detail',$order->order_tracking_id)}}">{{$order->user->name}}</a></td>
                        <td><a href="{{route('merchant.order.detail',$order->order_tracking_id)}}">{{$order->order_tracking_id}}</a></td>
                        <td><a href="{{route('merchant.order.detail',$order->order_tracking_id)}}" class="capitalize">{{$shipped}}</a></td>
                        <td>{{$order->payment}}</td>
                        <td>{{$total}}</td>
                    </tr>
                    @php $c++ @endphp
